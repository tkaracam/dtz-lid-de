...
check_rate_limit_json('dtz-tts-public', 30, 600);
...
function dtz_tts_normalize_text(string $text): string
{
...
function dtz_tts_normalize_speaker_tag(string $speaker): string
{
...
function dtz_tts_strip_leading_speaker_token(string $text): string
{
...
function dtz_tts_split_segments(string $script): array
{
    $normalized = dtz_tts_normalize_text($script);
    if ($normalized === '') {
        return [];
    }
    ...
    foreach ($speakerChunks as $chunk) {
        ...
        if (preg_match('/^((?:[A-D]|Sprecher\s*\d+|Speaker\s*\d+|Woman\s*[_-]?\s*\d+|Man\s*[_-]?\s*\d+|Moderator|Ansage|Narrator))\s*:\s*(.+)$/iu', $chunk, $m) === 1) {
            $speaker = dtz_tts_normalize_speaker_tag((string)$m[1]);
            $textBlock = dtz_tts_strip_leading_speaker_token((string)$m[2]);
        } else {
            $textBlock = dtz_tts_strip_leading_speaker_token($textBlock);
        }
...
function dtz_tts_voice_for_speaker(string $speaker, string $defaultVoice): string
{
...
function dtz_tts_cache_dir(): string
{
    return __DIR__ . '/storage/tts';
}

function dtz_tts_cache_filename(string $model, string $voice, string $text): string
{
    $hash = md5($model . '|' . $voice . '|' . $text);
    return $hash . '.mp3';
}

function dtz_tts_openai_generate(string $apiKey, string $model, string $voice, string $text): array
{
    $payload = [
        'model' => $model,
        'voice' => $voice,
...
$segments = dtz_tts_split_segments($script);
if (!$segments) {
    http_response_code(400);
...
foreach ($segments as $segment) {
    ...
    $voice = dtz_tts_voice_for_speaker($speaker, $defaultVoice);
    $file = dtz_tts_cache_filename($ttsModel, $voice, $text);
    $fullPath = $cacheDir . '/' . $file;

    if (!is_file($fullPath) || filesize($fullPath) < 1000) {
        $generated = dtz_tts_openai_generate($apiKey, $ttsModel, $voice, $text);
        if (!$generated['ok']) {
